...
function s.pencon1(e,c,og)
	...
	if og then
		return og:Filter(Card.IsLocation,nil,LOCATION_HAND):IsExists(Pendulum.Filter,1,nil,e,tp,lscale,rscale)
	else
...
function s.penop1(e,tp,eg,ep,ev,re,r,rp,c,sg,og)
	...
	if og then
		...
		local g=og:Filter(Card.IsLocation,nil,LOCATION_HAND):FilterSelect(tp,Pendulum.Filter,0,ft,nil,e,tp,lscale,rscale)
		if g then
			sg:Merge(g)
		end
	else
